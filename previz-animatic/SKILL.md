...
| `figures-cat.js` | 猫的另外三态：站立 / 走路循环（对角步态）/ 坐姿（可带骄傲表情、甩水抖动），支持三态嘴型 |
| `env.js` | 天空/云/远岸/河流/草地/柳树/飞鸟/阵风粒子（河堤环境积木）+ 雨（双层雨丝/涟漪/水冠）、白墙巷、便利店门脸、云裂金边、湿地金光斑 |
...
- 2026-09-16 E10 V2：侧脸"愤怒眉"的同根问题复发并根治——不光刘海三角
  挂到额头中部会读成倒八眉（bob/spikeV 刘海尖曾垂到 -0.42，紧挨眉线
  -0.34），默认眉形弧度过弯也会加重怒意。修复：刘海尖收到发际线
  （bob 0.35×len、spikeV 尖 -0.55），calm/gentle/curious 一律平眉微弯弧，
  **只有 alarmed 允许内端下压的倒八**。正面 drawBrows 原本就按表情分
  tilt，无此问题。另：正弦步态在相位过零的采样帧腿并拢读作站立——
  走路循环要有基础开腿量（step = sin(p)*0.11s + 0.05s）。
- 2026-09-16 E10：drawAlley 首版只铺了顶部一条天空色带，两面墙的屋檐线
  在灭点上方围出一块未着色的楔形——渲染出来是黑色大缺口。教训：环境积木
  **先满铺底色再往上叠形状**，不要假设后面的形状能严丝合缝盖住背景。
  同理 C1/C4 门面两侧、D3 三段式之间也靠满铺中景色带消灭缝隙。
