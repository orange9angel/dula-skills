...
13. **Human dialogue uses a hybrid channel; Seedance cannot hold character identity.** Verified by the E06-live channel shootout (2026-09-06): (a) Seedance I2V first-frame AND Seedance 2.5 multimodal reference share one moderation wall — uploaded photorealistic faces (AI-generated included) are rejected (`InputImageSensitiveContentDetected.PrivacyInformation`), so Seedance can never carry a series character's face; its pure-audio T2V gives superb naturalness (body/face 5/5) but identity 1/5 and weak directorial control — reserve Seedance 2.5 for non-face work (long takes, environments, animals, editing). (b) Default dialogue shots stay on OmniHuman (lip-sync 4/5, identity 5/5, ¥2–3/shot). (c) For key emotional shots where OmniHuman's stiffness shows, use DreamActor 2.0 human-mode: character photo + a natural front-face performance template (real filmed > Seedance 2.5 pure-audio T2V) — measured identity 5/5, body naturalness 4/5, lip-sync 3.5/5 (lip-sync decay comes from side-angle templates, not from DreamActor itself). Template angle discipline: front-face templates preserve lip-sync; side-angle templates trade it for motion.
