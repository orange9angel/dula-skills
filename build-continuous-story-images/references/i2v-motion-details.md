...
**I2V 色偏校正（E07 V3 实测）**：Seedance 满血版输出比首帧关键帧暗一截
（均值偏移 ~20%），剪辑点出现色系跳变。入片前逐段比邻居均值，偏差 >10%
就做 Reinhard 色板匹配（episodes/cat_leads_e07_river_willow/tools/
match_i2v_color.py）；注意先查首帧关键帧本身的亮度档——E07 的跳变一半
是关键帧自身（树荫场景）造成的，校正目标取邻居与首帧的均值中点最稳。
